...
function showToast(message, type = 'info') {
  ...
  let borderClass = 'border-blue-500/30 bg-blue-50 text-blue-900';
  let icon = 'info';

  if (type === 'success') {
    borderClass = 'border-emerald-500/30 bg-emerald-50 text-emerald-900';
    icon = 'check-circle';
  } else if (type === 'error') {
    borderClass = 'border-red-500/30 bg-red-50 text-red-900';
    icon = 'alert-triangle';
  } else if (type === 'warning') {
    borderClass = 'border-amber-500/30 bg-amber-50 text-amber-900';
    icon = 'alert-circle';
  }

  toast.className = `toast-item flex items-start gap-2.5 px-4 py-3 rounded-lg border ${borderClass} shadow-lg text-sm font-bold opacity-0 translate-y-2 pointer-events-auto bg-brand-card transition-all duration-300`;
...
function confirmDialog({ title = 'Confirmar ação', message = '', confirmLabel = 'Confirmar', cancelLabel = 'Cancelar', danger = false } = {}) {
  return new Promise((resolve) => {
    ...
    if(danger) {
      ...
      iconWrap.className = 'w-10 h-10 rounded-lg flex items-center justify-center mx-auto mb-4 bg-red-100 text-red-700';
...
function calculateOrderDeadline(deadlineStr, orderStatus) {
  if (orderStatus === 'Entregue') return { label: 'Entregue', class: 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-900 border border-emerald-200' };
  const today = getTodayIsoString();
  if (!deadlineStr) return { label: 'Sem prazo', class: 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-100 text-slate-900 border border-slate-200' };
  const cleanDeadline = String(deadlineStr).split('T')[0];
  if (cleanDeadline < today) return { label: 'Atrasado', class: 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-900 border border-red-200' };
  if (cleanDeadline === today) return { label: 'Entrega hoje', class: 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-900 border border-amber-200' };
  return { label: 'No prazo', class: 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-blue-100 text-blue-900 border border-blue-200' };
}
  ...
  tabs.forEach(t => {
    ...
    if (btn) {
      btn.className = t === tabName
        ? "nav-tab w-full px-3 py-2.5 rounded-lg text-sm font-bold text-brand-dark bg-brand-slate transition-all flex items-center gap-3 shadow-sm border border-brand-border"
        : "nav-tab w-full px-3 py-2.5 rounded-lg text-sm font-bold text-slate-900 hover:text-brand-dark hover:bg-brand-slate transition-all flex items-center gap-3";
      btn.setAttribute('aria-current', t === tabName ? 'page' : 'false');
    }
    if (mBtn) {
      mBtn.className = t === tabName
        ? "flex flex-col items-center text-[10px] text-brand-dark font-black py-1"
        : "flex flex-col items-center text-[10px] text-slate-900 font-bold py-1";
    }
  });
...
function renderAlertBanner(atrasados, hoje) {
  ...
  box.innerHTML = `
    <div class="flex items-center gap-3">
      <div class="w-10 h-10 rounded-lg bg-amber-100 text-amber-900 flex items-center justify-center shrink-0">
        <i data-lucide="alert-triangle" class="w-5 h-5"></i>
      </div>
      <div>
        <h4 class="text-sm font-black text-amber-950">Atenção aos Prazos</h4>
        <p class="text-xs text-amber-900 font-bold mt-0.5">
          ${atrasados > 0 ? `<strong>${atrasados} pedido(s) atrasado(s)</strong>. ` : ''}
          ${hoje > 0 ? `<strong>${hoje} entrega(s) para hoje</strong>.` : ''}
        </p>
      </div>
    </div>
    <button onclick="switchTab('pedidos')" class="text-xs font-bold px-4 py-2 rounded-lg bg-brand-dark text-brand-cream hover:bg-slate-800 transition-colors shadow-sm">
      Ver Pedidos
    </button>`;
...
function updateCharts(noPrazo, hoje, atrasado, entregue) {
    ...
    financialChartInstance = new Chart(ctxFin, {
      ...
      data: {
        ...
        datasets: [
          ...
          { label: 'Valor Pago (R$)', data: dataPaid.length ? dataPaid : [0], backgroundColor: '#059669CC', borderColor: '#059669', borderWidth: 1.5, borderRadius: 4 }
        ]
      },
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: { legend: { labels: { color: mutedColor, font: { family: 'Inter', size: 11, weight: 'bold' } } } },
        scales: {
          x: { grid: { color: gridColor }, ticks: { color: mutedColor, font: { weight: 'bold' } } },
          y: { grid: { color: gridColor }, ticks: { color: mutedColor, font: { weight: 'bold' }, callback: (v) => 'R$ ' + v } }
        }
      }
    });
  }
  ...
  if (ctxStat) {
    ...
    statusChartInstance = new Chart(ctxStat, {
      ...
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: { legend: { position: 'bottom', labels: { color: mutedColor, font: { family: 'Inter', size: 11, weight: 'bold' } } } },
        cutout: '70%'
      }
    });
  }
...
function renderOrdersTable() {
  ...
  filtered.forEach(order => {
    ...
    let payBadgeClass = "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-900 border border-red-200";
    if (payStatus === 'Pago') payBadgeClass = "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-900 border border-emerald-200";
    if (payStatus === 'Parcial') payBadgeClass = "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-900 border border-amber-200";

    const tr = document.createElement('tr');
    tr.className = "hover:bg-brand-slate/50 transition-colors";
    tr.innerHTML = `
      <td class="py-3 px-4 text-xs">
        <span class="font-black font-mono text-brand-dark cursor-pointer hover:underline text-sm" onclick="openOrderDetailsModal('${order.id}')">${escapeHtml(order.code)}</span>
        <span class="block text-[11px] font-bold text-slate-900 mt-0.5 truncate max-w-[180px]">${escapeHtml(order.category)}</span>
      </td>
      <td class="py-3 px-4 font-black text-brand-dark text-sm">
        ${client ? escapeHtml(client.name) : '<span class="text-red-700">Removido</span>'}
      </td>
      <td class="py-3 px-4 text-xs font-bold text-slate-900">
        <span class="inline-flex items-center px-2.5 py-1 rounded bg-brand-slate border border-brand-border text-[11px] font-bold text-slate-900">${escapeHtml(order.status)}</span>
      </td>
      <td class="py-3 px-4 text-xs">
        <div class="font-black text-brand-dark">${formatDate(order.deadline)}</div>
        <span class="mt-1 ${dl.class}">${dl.label}</span>
      </td>
      <td class="py-3 px-4 text-right font-black text-brand-dark text-sm">${formatBRL(order.totalValue)}</td>
      <td class="py-3 px-4 text-right font-black text-emerald-700 text-sm">${formatBRL(paid)}</td>
      <td class="py-3 px-4 text-right font-black ${remaining > 0 ? 'text-amber-700' : 'text-slate-500'} text-sm">${formatBRL(remaining)}</td>
      <td class="py-3 px-4 text-center">
        <span class="${payBadgeClass}">${payStatus}</span>
      </td>
      <td class="py-3 px-4 text-center">
        <div class="flex items-center justify-center gap-1.5">
          <button onclick="openOrderDetailsModal('${order.id}')" title="Ver Detalhes" class="p-1.5 rounded hover:bg-brand-border text-slate-900 hover:text-brand-dark transition font-bold"><i data-lucide="eye" class="w-4 h-4"></i></button>
          <button onclick="openPaymentForOrder('${order.id}')" title="Registrar Pagamento" class="p-1.5 rounded hover:bg-amber-100 text-amber-900 transition font-bold"><i data-lucide="dollar-sign" class="w-4 h-4"></i></button>
          <button onclick="editOrder('${order.id}')" title="Editar Pedido" class="p-1.5 rounded hover:bg-brand-border text-slate-900 hover:text-brand-dark transition font-bold"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
          <button onclick="deleteOrder('${order.id}')" title="Excluir" class="p-1.5 rounded hover:bg-red-100 text-red-700 transition font-bold"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
        </div>
      </td>`;
...
function renderClientsTable() {
  ...
  filtered.forEach(client => {
    ...
    tr.innerHTML = `
      ...
      <td class="py-3 px-4 text-center font-black text-brand-dark text-sm">${ordersCount}</td>
      <td class="py-3 px-4 text-right font-black text-brand-dark text-sm">${formatBRL(totalBought)}</td>
      <td class="py-3 px-4 text-right font-black text-emerald-700 text-sm">${formatBRL(totalPaid)}</td>
      <td class="py-3 px-4 text-right font-black ${balance > 0 ? 'text-amber-700' : 'text-slate-500'} text-sm">${formatBRL(balance)}</td>
      <td class="py-3 px-4 text-center">
        ${waLink
          ? `<a href="${waLink}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-900 border border-emerald-200 hover:bg-emerald-200 transition"><i data-lucide="message-square" class="w-3 h-3"></i> WhatsApp</a>`
          : `<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-900 border border-slate-200">Sem telefone</span>`}
      </td>
      <td class="py-3 px-4 text-center">
        <div class="flex items-center justify-center gap-1.5">
          <button onclick="openMeasurementsModal('${client.id}')" title="Medidas & PDF" class="p-1.5 rounded hover:bg-amber-100 text-amber-900 transition font-bold"><i data-lucide="ruler" class="w-4 h-4"></i></button>
          <button onclick="editClient('${client.id}')" title="Editar" class="p-1.5 rounded hover:bg-brand-border text-slate-900 hover:text-brand-dark transition font-bold"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
          <button onclick="deleteClient('${client.id}')" title="Excluir" class="p-1.5 rounded hover:bg-red-100 text-red-700 transition font-bold"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
        </div>
      </td>`;
    tbody.appendChild(tr);
  });
...
function renderPaymentsTable() {
  ...
  sorted.forEach(pay => {
    ...
    tr.className = "hover:bg-brand-slate/50 transition-colors";
    tr.innerHTML = `
      <td class="py-3 px-4 text-xs font-mono text-slate-900 font-bold">${formatDate(pay.date)}</td>
      <td class="py-3 px-4 font-mono text-xs font-black text-brand-dark">${order ? escapeHtml(order.code) : 'Avulso'}</td>
      <td class="py-3 px-4 font-black text-brand-dark text-sm">${client ? escapeHtml(client.name) : '-'}</td>
      <td class="py-3 px-4"><span class="inline-flex items-center px-2 py-0.5 rounded bg-brand-slate border border-brand-border text-[11px] font-bold text-slate-900">${escapeHtml(pay.method)}</span></td>
      <td class="py-3 px-4 text-xs text-slate-900 font-bold truncate max-w-[200px]">${escapeHtml(pay.notes || '-')}</td>
      <td class="py-3 px-4 text-right font-black text-emerald-700 text-sm">${formatBRL(pay.amount)}</td>
      <td class="py-3 px-4 text-center">
        <div class="flex items-center justify-center gap-1.5">
          <button onclick="editPayment('${pay.id}')" title="Editar" class="p-1.5 rounded hover:bg-brand-border text-slate-900 hover:text-brand-dark transition font-bold"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
          <button onclick="deletePayment('${pay.id}')" title="Excluir" class="p-1.5 rounded hover:bg-red-100 text-red-700 transition font-bold"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
        </div>
      </td>`;
    tbody.appendChild(tr);
  });
...
function generateMeasurementsPDF() {
  ...
  let html = `
    ...
    <h2 style="font-size: 22px; font-weight: bold; margin-bottom: 10px; color: #0F1826;">Ficha de Medidas</h2>
    <p style="font-size: 14px; margin-bottom: 30px; line-height: 1.5; color: #0F1826;">
  ...
  const addSection = (title, content) => {
    ...
    html += `
      <div style="margin-bottom: 25px;">
        ...
        <div style="font-size: 14px; line-height: 1.6; white-space: pre-wrap; padding-left: 12px; color: #0F1826;">${text}</div>
      </div>
    `;
  }
  ...
  if (obs.trim()) {
     html += `
      <div style="margin-top: 40px; border-top: 1px dashed #C5B288; padding-top: 20px;">
        ...
        <div style="font-size: 14px; line-height: 1.6; white-space: pre-wrap; color: #0F1826;">${escapeHtml(obs)}</div>
      </div>
    `;
  }
...
function openOrderDetailsModal(orderId) {
  ...
  if (orderPayments.length === 0) {
    listContainer.innerHTML = '<p class="text-xs text-slate-900 font-bold italic">Nenhum pagamento registrado.</p>';
  } else {
    orderPayments.forEach(p => {
      ...
      item.innerHTML = `
        <div>
          <span class="font-black text-brand-dark">${formatDate(p.date)}</span> — 
          <span class="inline-flex items-center px-2 py-0.5 rounded bg-white border border-brand-border text-[11px] font-bold text-slate-900">${escapeHtml(p.method)}</span>
          ${p.notes ? `<span class="text-slate-900 block text-[11px] mt-1 font-bold">${escapeHtml(p.notes)}</span>` : ''}
        </div>
        <div class="flex items-center gap-3">
          <span class="font-black text-emerald-700">${formatBRL(p.amount)}</span>
          <div class="flex items-center gap-1 border-l border-brand-border pl-2">
             <button onclick="editPayment('${p.id}')" class="p-1 text-slate-900 hover:text-brand-dark transition font-bold"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
             <button onclick="deletePayment('${p.id}')" class="p-1 text-slate-900 hover:text-red-700 transition font-bold"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
          </div>
        </div>`;
