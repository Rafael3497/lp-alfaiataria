...
function calculateOrderDeadline(deadlineStr, orderStatus) {
  ...
  if (!deadlineStr) return { label: 'Sem prazo', class: 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200' };
  ...
  tabs.forEach(t => {
    ...
    if (btn) {
      btn.className = t === tabName
        ...
        : "nav-tab w-full px-3 py-2.5 rounded-lg text-sm font-medium text-slate-600 hover:text-brand-dark hover:bg-brand-slate transition-all flex items-center gap-3";
    ...
    if (mBtn) {
      mBtn.className = t === tabName
        ...
        : "flex flex-col items-center text-[10px] text-slate-600 font-medium py-1";
    }
  });
  ...
  filtered.forEach(order => {
    ...
    tr.innerHTML = `
      <td class="py-3 px-4 text-xs">
        ...
        <span class="block text-[11px] font-medium text-slate-500 mt-0.5 truncate max-w-[180px]">${escapeHtml(order.category)}</span>
      </td>
      <td class="py-3 px-4 font-medium text-brand-dark">
      ...
      <td class="py-3 px-4 text-xs">
        <div class="font-medium text-brand-dark">${formatDate(order.deadline)}</div>
        <span class="mt-1 ${dl.class}">${dl.label}</span>
      </td>
      <td class="py-3 px-4 text-right font-medium text-brand-dark">${formatBRL(order.totalValue)}</td>
      <td class="py-3 px-4 text-right font-medium text-emerald-700">${formatBRL(paid)}</td>
      <td class="py-3 px-4 text-right font-semibold ${remaining > 0 ? 'text-amber-700' : 'text-slate-400'}">${formatBRL(remaining)}</td>
      <td class="py-3 px-4 text-center">
        <span class="${payBadgeClass}">${payStatus}</span>
      </td>
      <td class="py-3 px-4 text-center">
        <div class="flex items-center justify-center gap-1.5">
          <button onclick="openOrderDetailsModal('${order.id}')" title="Ver Detalhes" class="p-1.5 rounded hover:bg-brand-border text-slate-600 hover:text-brand-dark transition"><i data-lucide="eye" class="w-4 h-4"></i></button>
          ...
          <button onclick="editOrder('${order.id}')" title="Editar Pedido" class="p-1.5 rounded hover:bg-brand-border text-slate-600 hover:text-brand-dark transition"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
    ...
    tr.innerHTML = `
      <td class="py-3 px-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="font-medium text-brand-dark">${escapeHtml(client.name)}</div>
            <div class="text-xs text-slate-500 font-mono">${escapeHtml(client.phone || '-')}</div>
            ${client.email ? `<div class="text-[11px] text-slate-400">${escapeHtml(client.email)}</div>` : ''}
          </div>
        </div>
      </td>
      <td class="py-3 px-4 text-center font-medium text-brand-dark">${ordersCount}</td>
      <td class="py-3 px-4 text-right font-medium text-brand-dark">${formatBRL(totalBought)}</td>
      <td class="py-3 px-4 text-right font-medium text-emerald-700">${formatBRL(totalPaid)}</td>
      <td class="py-3 px-4 text-right font-semibold ${balance > 0 ? 'text-amber-700' : 'text-slate-400'}">${formatBRL(balance)}</td>
      <td class="py-3 px-4 text-center">
        ${waLink
          ...
          : `<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200">Sem telefone</span>`}
      </td>
      <td class="py-3 px-4 text-center">
        <div class="flex items-center justify-center gap-1.5">
          ...
          <button onclick="editClient('${client.id}')" title="Editar" class="p-1.5 rounded hover:bg-brand-border text-slate-600 hover:text-brand-dark transition"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
...
function renderPaymentsTable() {
  ...
  sorted.forEach(pay => {
    ...
    tr.className = "hover:bg-brand-slate/50 transition-colors";
    tr.innerHTML = `
      <td class="py-3 px-4 text-xs font-mono text-slate-600">${formatDate(pay.date)}</td>
      <td class="py-3 px-4 font-mono text-xs font-semibold text-brand-dark">${order ? escapeHtml(order.code) : 'Avulso'}</td>
      <td class="py-3 px-4 font-medium text-brand-dark">${client ? escapeHtml(client.name) : '-'}</td>
      <td class="py-3 px-4"><span class="inline-flex items-center px-2 py-0.5 rounded bg-brand-slate border border-brand-border text-[11px] font-medium text-slate-700">${escapeHtml(pay.method)}</span></td>
      <td class="py-3 px-4 text-xs text-slate-500 truncate max-w-[200px]">${escapeHtml(pay.notes || '-')}</td>
      <td class="py-3 px-4 text-right font-semibold text-emerald-700">${formatBRL(pay.amount)}</td>
      <td class="py-3 px-4 text-center">
        <div class="flex items-center justify-center gap-1.5">
          <button onclick="editPayment('${pay.id}')" title="Editar" class="p-1.5 rounded hover:bg-brand-border text-slate-600 hover:text-brand-dark transition"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
...
function openOrderDetailsModal(orderId) {
  ...
  if (orderPayments.length === 0) {
    listContainer.innerHTML = '<p class="text-xs text-slate-500 italic">Nenhum pagamento registrado.</p>';
  } else {
    orderPayments.forEach(p => {
      ...
      item.innerHTML = `
        <div>
          <span class="font-medium text-brand-dark">${formatDate(p.date)}</span> — 
          <span class="inline-flex items-center px-2 py-0.5 rounded bg-white border border-brand-border text-[11px] font-medium text-slate-700">${escapeHtml(p.method)}</span>
          ${p.notes ? `<span class="text-slate-500 block text-[11px] mt-1">${escapeHtml(p.notes)}</span>` : ''}
        </div>
        <div class="flex items-center gap-3">
          ...
          <div class="flex items-center gap-1 border-l border-brand-border pl-2">
             <button onclick="editPayment('${p.id}')" class="p-1 text-slate-500 hover:text-brand-dark transition"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
             <button onclick="deletePayment('${p.id}')" class="p-1 text-slate-500 hover:text-red-700 transition"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
          </div>
        </div>`;
